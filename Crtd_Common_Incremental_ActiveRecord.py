...
# MAGIC %run /THAILAND/UCRM/99_Configuration/UCRM_UDF

# COMMAND ----------

# MAGIC %run /THAILAND/UCRM/99_Configuration/UCRM_Parameters
...
fullSqlString = """
  ...
  from
  (
  select {0},row_number() over(partition by {1} order by ID_Key desc) as rownum
  from {2}.{3} stg
  )
  where rownum = 1""".format(selectStatement,pkConcatStg,structuredSchema,sourceTableName)
